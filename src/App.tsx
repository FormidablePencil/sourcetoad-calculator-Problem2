...
function App() {
  const Box = ({ item, orangeColor }: { item: string, orangeColor: boolean }) =>
    <div className="flex-1 px-2 py-2 justify-center flex items-center text-white text-2xl font-semibold">
      <div className={`rounded-full h-20 w-20 flex items-center ${orangeColor ? "bg-orange-500" : "bg-purple-800"} justify-center shadow-lg border-2 border-purple-700 hover:border-2 hover:border-gray-500 focus:outline-none`}>{item}</div>
    </div>
  ...
  return (
    <div className="flex h-screen bg-fuchsia-900 justify-center items-center">
      <div className="mx-auto overflow-hidden mt-10 mb-2 bg-purple-900 shadow-lg  rounded-lg md:w-3/6 sm:w-4/6 pb-6">
        ...
        <div className="p-5 text-white text-right text-3xl bg-purple-800">= <span className="text-orange-500">2100</span></div>
